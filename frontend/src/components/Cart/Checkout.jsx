    ...
    const handleCreateCheckout = async (e)=>{
           ...
           if(cart && cart.products.length > 0){
            ...
            if(res.payload && res.payload._id){
              setCheckoutId(res.payload._id) // set checkout id if checkout was successfully
            }
           }    
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl max-auto py-10 px-6 tracking-tighter">
      ...
      <div className="bg-white rounded-sm p-6">
           <h2 className="text-2xl uppercase mb-6 text-black">Checkout</h2>
           <form onSubmit={handleCreateCheckout}>
                ...
                <h3 className="text-lg mb-4 text-gray-800">Delivery</h3>
                <div className="mb-4 grid grid-cols-2 gap-4">
                   <div>
                      <label className="block text-gray-600">First Name</label>
                      <input value={shippingAddress.firstName} onChange={(e)=>{ setShippingAddress({...shippingAddress, firstName: e.target.value})}} type="text" className="w-full p-2 border rounded" required />
                   </div>
                   <div>
                      <label className="block text-gray-600">Last Name</label>
                      <input value={shippingAddress.lastName} onChange={(e)=>{ setShippingAddress({...shippingAddress, lastName: e.target.value})}} type="text" className="w-full p-2 border rounded" required />
                   </div>
                </div>
                <div className="mb-4">
                ...
                <div className="mb-4 grid grid-cols-2 gap-4">
                   ...
                   <div>
                      ...
                      <input value={shippingAddress.postalCode} onChange={(e)=>{ setShippingAddress({...shippingAddress, postalCode: e.target.value})}} type="text" className="w-full p-2 border rounded" required />
                   </div>
                </div>
                <div className="mb-4">
                ...
                <div className="mt-6">
                  {!checkoutId ? (
                    ...
                    <div>
                       <h3 className="text-lg mb-4 text-gray-8800">Pay with PayPal</h3>
      ...
      <div className="bg-gray-50 p-6 rounded-sm">
         ...
         <div className="text-lg mb-4">
          {cart.products.map((product,index)=>(
            <div key={index} className="flex items-start justify-between py-2 border-lg">
              <div className="flex items-start">
                <img src={product.image} alt={product.name} className="w-20 h-24 object-cover mb-4 rounded-sm" />
                <div className="ml-4">
